...
interface DiaryContext {
  ...
  imageBase64: string;
  style?: 'emotional' | 'factual' | 'brief' | 'dramatic';
}
    ...
    style === 'emotional'
      ...
      : style === 'factual'
        ...
        : style === 'brief'
          ...
          : `당신은 부모가 아이에게 남기는 육아 일기를 영화 내레이션처럼 드라마틱하게 써주는 AI입니다. 그날의 평범한 순간을 극적인 서사로 재구성하고, 영화 같은 클라이맥스와 긴장감 있는 문장을 사용하세요. "그 순간, 세상이 멈췄다", "운명처럼", "마치 영원 같았다" 같은 극적인 표현을 자유롭게 활용하되, "너는", "${ctx.childName}은" 같이 아이에게 말을 건네는 문체로 쓰고, 마크다운 기호(**,##,- 등)는 절대 사용하지 마세요.`;
...
- ${ctx.childName}의 아주 작은 몸짓 하나도 영화 속 결정적 장면처럼 묘사하세요
- 남은 날수나 현재 날짜를 언급하지 마세요 — 사진 속 순간 자체에 집중하세요
- 150~250자 내외로 작성해주세요`
